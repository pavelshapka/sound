...
SoundDataset = namedtuple('SoundDatset', ['mu', 'Cp', 'Cv'])
...
gamma = Cp / Cv
mu = air.mu * xAir + water.mu * xH2O + co2.mu * xCO2

s_speed = np.sqrt(gamma * R * temperature / mu)
...
n_air = (speed_air - p[1])/p[0]
n_breath = (speed_breath - p[1])/p[0]
...
plt.legend(['Аналитическая зависимость', 'Значение в воздухе: {:.2f} [м/с], {:.2f} [%]'.format(speed_air, n_air),
            'Значения в выдохе: {:.2f} [м/с], {:.2f} [%]'.format(speed_breath, n_breath)], loc='upper right')
plt.show()
